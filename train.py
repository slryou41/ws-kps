...
from utils import Logger, show_img_with_pts, mkdir_p, show_heatmaps, show_img_with_heatmap, show_img_with_confidence
import numpy as np
...
parser = argparse.ArgumentParser(description='PyTorch Keypoint Discovery Training')
...
parser.add_argument('--schedule', default=50, type=int, metavar='N')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    model = Model(args.nkpts, output_dim=args.nclass) #, cls_kpts=args.nkpts)
    ...
    class_crit = nn.CrossEntropyLoss().cuda(args.gpu)
    mse_crit = nn.MSELoss().cuda(args.gpu)
    
    criterion = [class_crit, mse_crit, recon_crit]
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if os.path.isfile(os.path.join(args.checkpoint, 'log.txt')):
                ...
                logger.set_names(['Learning Rate', 'Train Loss', 'Train Acc', 'Valid Loss', 'Valid Acc',])
        else:
    ...
    if args.dataset == 'cub':
        train_dataset = cub.CUBRefined(args.data, True, is_train=True, image_size=[128, 128], 
                                       data_ratio=1.0, return_grid=True)
        val_dataset = cub.CUBRefined(args.data, False, is_train=True, evaluation=False,
                                     image_size=[128, 128], data_ratio=1.0, return_grid=True)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_loss, train_acc = train(train_loader, model, loss_model, criterion, optimizer, epoch, args)

        # evaluate on validation set
        test_loss, acc1 = validate(val_loader, model, criterion, epoch, args)
        
        logger.append([args.lr * (0.1 ** (epoch // args.schedule)), train_loss, train_acc, test_loss, acc1])
...
# def train(train_loader, model, loss_model, criterion, optimizer, epoch, pseudo_model, args):
    ...
    if debug:
        ...
        _mean = np.asarray(_mean).reshape((3,1,1))
        _std=[0.229, 0.224, 0.225]
        _std = np.asarray(_std).reshape((3,1,1))
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            ...
            grid, future_grid = images[4].cuda(args.gpu, non_blocking=True), \
        ...
        if loss_model is not None:
            ...
            vgg_feat_in = loss_model(tr_inputs)
            vgg_feat_out = loss_model(output[1])
        ...
        if epoch > 30:
            # Pick samples with high x variance
            # Based on class keypoints direction
            x_var = torch.var(output[8][0], dim=1)
            _, x_ind = torch.sort(x_var, descending=True)
            sample_idx = x_ind[:20]

            kp_direction = output[8][0][sample_idx][:,:3].mean(dim=1)
            _, kp_d_ind = torch.sort(kp_direction, descending=True)
            sample_idx = sample_idx[kp_d_ind[:10]]

            flipped_input = copy.deepcopy(inputs.flip(-1))
            pseudo_output = model(flipped_input, tr_inputs, pseudo_run=True)
...
def save_checkpoint(state, is_best, checkpoint='checkpoint', filename='checkpoint.pth.tar', epoch=None):
    filepath = os.path.join(checkpoint, filename)
    torch.save(state, filepath)
    if epoch == 29:
        torch.save(state, os.path.join(checkpoint, 'class_checkpoint.pth.tar'))
    if is_best:
...
def bce_accuracy(output, target, topk=(1,)):
    with torch.no_grad():
        maxk = max(topk)
        batch_size = target.size(0)
        ...
        correct_k = (pred == target).view(-1).float().sum(0, keepdim=True)
        res = correct_k.mul_(100.0 / (batch_size*target.size(1)))
        res = [res, res]
